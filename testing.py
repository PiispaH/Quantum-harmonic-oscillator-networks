...
cov_X_initial = np.block([[np.diag(Q2_expectation * Q2_multiplier), zero],
                          [zero, np.diag(P2_expectation * P2_multiplier)]])
...
def main():
    ...
    cov = qt.Qobj(inpt=cov_X_initial)
    print(cov)

    # Wigner function
    ax_range = np.linspace(-5,5,200)
    w = qt.wigner(cov, ax_range, ax_range)
    wigner_visualization(w, ax_range)
    print(w)
